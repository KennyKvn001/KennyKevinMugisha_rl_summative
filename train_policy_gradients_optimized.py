...
class DetailedTrainingLogger(BaseCallback):
        ...
        if self.locals.get("dones", [False])[0]:
            ...
            if "episode" in info:
                ...
                if self.episode_count % self.log_interval == 0:
                    ...
                    recent_goals = self.episode_goals[-50:] if len(self.episode_goals) >= 50 else self.episode_goals
                    ...
                    print(f"  Avg Reward: {avg_reward:.2f} | Avg Goals: {avg_goals:.2f} | Avg Length: {avg_length:.1f}")
                    print(f"  Best Reward: {self.best_reward:.2f} | Best Goals: {self.best_goals}")
                    print(f"  Time Elapsed: {elapsed_time/60:.1f}min")
                    
                    # Success rate
                    if len(recent_goals) > 0:
                        success_rate = sum(1 for g in recent_goals if g >= 3) / len(recent_goals) * 100
    ...
    def get_training_stats(self):
        ...
        return {
            ...
            'final_avg_goals': np.mean(self.episode_goals[-100:]) if len(self.episode_goals) >= 100 else np.mean(self.episode_goals) if self.episode_goals else 0,
...
def train_ppo_optimized():
    ...
    env = DenseRewardWrapper(base_env, dense_reward_scale=0.5)  # Scale down dense rewards
...
def train_reinforce_optimized():
    ...
    # Create environment with dense rewards
    base_env = create_env(training_mode=True)
    env = DenseRewardWrapper(base_env, dense_reward_scale=0.2)  # Lower scale for REINFORCE
    ...
    return policy, {
        ...
        'final_avg_goals': np.mean(episode_goals[-100:]) if len(episode_goals) >= 100 else np.mean(episode_goals),
...
def create_comparison_plots(training_stats):
    ...
    fig.suptitle('Policy Gradient Algorithms Performance Comparison', fontsize=16, fontweight='bold')
    ...
    for i, (algo, stats) in enumerate(training_stats.items()):
        ...
        if len(rewards) >= window:
            ...
            ax1.plot(range(window-1, len(rewards)), smoothed, label=algo, color=colors[i], linewidth=2)
        else:
    ...
    for i, (algo, stats) in enumerate(training_stats.items()):
        ...
        if len(goals) >= window:
            ...
            ax2.plot(range(window-1, len(goals)), smoothed, label=algo, color=colors[i], linewidth=2)
        else:
    ...
    for i, (algo, stats) in enumerate(training_stats.items()):
        ...
        if len(lengths) >= window:
            ...
            ax3.plot(range(window-1, len(lengths)), smoothed, label=algo, color=colors[i], linewidth=2)
        else:
    ...
    for bar, value in zip(bars, final_rewards):
        ax4.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(final_rewards)*0.01,
    ...
    ax6 = axes[1, 2]
    training_times = [stats['training_time']/60 for stats in training_stats.values()]  # Convert to minutes
...
def main():
    ...
    try:
        ...
        print("python evaluate_agent.py --algo ppo --model_path model_ppo_dense.zip --episodes 5")
        print("python evaluate_agent.py --algo a2c --model_path model_a2c_dense.zip --episodes 5")
        print("python evaluate_agent.py --algo reinforce --model_path model_reinforce_dense.pth --episodes 5")
